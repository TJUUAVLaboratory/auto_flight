...
flying_test::flying_test(string cam_params) : imp(cam_params),
                                              framePro((imp.matQ)),
                                              it(nh),
...
void flying_test::slam()
{
    ...
    while (ros::ok()) //&&!traj_planning.arrive_destination(traj_planning.get_trans_msg_a(),traj_planning.get_destinate_coor())
    {
        ...
        // if (imp.image_num == framePro.image_pose_num)
        // {
...
{
    ros::init(argc, argv, "mavros_flying_test");

    ros::NodeHandle private_nh("~");
    string cam_params;
    private_nh.param("cam_params", cam_params, std::string("src/auto_flight/default.yaml"));

    flying_test mavros_flying_test(cam_params);
    mavros_flying_test.muti_thread();
